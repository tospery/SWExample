...
open class ExampleViewController: BaseViewController {
    
    let reuseID = "CaseCell"
    let disposeBag = DisposeBag()
    
    lazy var dataSource: RxTableViewSectionedReloadDataSource<ExampleSection> = {
    ...
    open override func viewDidLoad() {
        ...
        if let path = Bundle.main.path(forResource: name, ofType: "json"),
            ...
            let cases = Array<Example>(JSONString: string) {
            sections = [
                ExampleSection(items: cases)
            ]
        }
